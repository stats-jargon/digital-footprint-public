  ...
  sidebarLayout(
    sidebarPanel(
      ...
      fluidRow(
        ...
        column(6, actionButton("forecast", "Run Forecast"))
      ),
      tags$br(),  # Adds extra space below
...
server <- function(input, output, session) {
  ...
  forecastData <- eventReactive(input$forecast, {
    ...
    weekly <- data.frame(ds_ts = forecast$ds_ts, weekly = forecast$weekly)
    ...
    list(forecasted_data = forecasted_data, actual_data = actual_data, table_data = table_data,
         model = model, trend = trend, weekly = weekly, yearly = yearly)
  })
  ...
  # Render interactive weekly seasonality plot with formatted date
  output$weeklyPlot <- renderHighchart({
    req(forecastData())
    weekly <- forecastData()$weekly
    
    highchart() %>%
      hc_title(text = "Weekly Seasonality Component") %>%
      hc_xAxis(type = "datetime", labels = list(format = "{value:%Y-%m-%d}")) %>%
      hc_add_series(name = "Weekly Seasonality", data = list_parse2(weekly), type = "line") %>%
      hc_tooltip(xDateFormat = "%Y-%m-%d", pointFormat = "{series.name}: <b>{point.y}</b>")
  })
